...
export function ImportWalletModal({
  ...
  return (
    <BaseModal isOpen={isOpen} onClose={onClose} title="Import your wallets" icon="/modal/import.gif">
      <div className="flex flex-col gap-6 p-2 bg-[#1E1E1E] rounded-b-2xl w-[600px]">
        {/* File Upload Area */}
        <div
          className={`relative border-2 border-dashed rounded-2xl p-2 text-center transition-all duration-200 ${
            isDragOver ? "border-blue-500 bg-blue-500/10 border-solid" : "border-[#292929] bg-[#131313]"
          }`}
        ...
        >
          ...
          <ActionButton
            ...
            className="mx-auto w-[150px] h-[40px] text-lg py-3 mb-3"
            type="neutral"
          />
          ...
          {selectedFiles.length > 0 && !isImporting && (
            <div className="mt-3 text-left">
              ...
              {selectedFiles.map((file, index) => (
                <div key={index} className="text-sm text-blue-400 bg-[#333] rounded px-2 py-1 mb-1">
                  {file.name}
                </div>
              ))}
            </div>
        ...
        {/* Tooltip */}
        <div className="bg-[#4A90A4]/25 border border-[#5B9FB3] rounded-2xl p-4">
          <div className="flex gap-3 items-center">
            <div className="flex-shrink-0 mt-0.5">
              <div className="w-6 h-6 bg-white rounded-full flex items-center justify-center">
                <img src="/connect-wallet/warning.svg" alt="" />
              </div>
            </div>
