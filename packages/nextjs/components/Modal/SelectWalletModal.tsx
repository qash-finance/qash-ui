...
const WalletItem = ({ label, icon, iconClassName }: { label: string; icon: string; iconClassName?: string }) => {
  return (
    <div className="flex justify-between items-center bg-app-background border-t-2 border-primary-divider w-full rounded-2xl p-3 cursor-pointer">
...
export function SelectWalletModal({ isOpen, onClose, zIndex }: ModalProp<ValidatingModalProps>) {
  ...
  return (
    <BaseModal isOpen={isOpen} onClose={onClose} title="Select Wallet" icon="/gift/gift-icon.svg" zIndex={zIndex}>
      ...
      <div className="bg-background flex flex-col rounded-b-2xl justify-center items-center py-5 px-3 gap-7">
        ...
        <div className="flex flex-col gap-4 w-full">
          <WalletItem label="Miden" icon="/logo/miden.svg" iconClassName="w-10" />
          <WalletItem label="MetaMask" icon="/logo/metamask.svg" />
          <WalletItem label="Phantom" icon="/logo/phantom.svg" />
          <WalletItem label="Coinbase Wallet" icon="/logo/coinbase.svg" />
        </div>
