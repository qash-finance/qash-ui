...
export interface PayrollActionTooltipProps {
  onEdit?: () => void;
  onDuplicate?: () => void;
  onRemove?: () => void;
}
...
const PayrollActionTooltip: React.FC<PayrollActionTooltipProps> = ({ onEdit, onDuplicate, onRemove }) => {
  return (
    <div className="bg-background relative rounded-2xl w-50 shadow-sm border border-primary-divider flex flex-col">
      ...
      <button className={`${itemStyles} rounded-t-2xl`} onClick={onEdit}>
        ...
        <span className="font-medium text-sm text-text-primary whitespace-nowrap">Edit payroll</span>
      </button>
...
export default PayrollActionTooltip;
